...
struct Gantt{ // for producing Gantt Chart
	...
	Gantt() : preIdle(true), numCPUContextSwitch(-1){} // constructor. numCPUContextSwitch set to -1 because when the very first process gets pushed to CPU, that does not count as a context switch
	void reset(); // reset all parameters
};

// output
void printGanttChart(const Gantt& gantt); // print out Gantt Chart
void printRT_WT_TT(const std::vector<Process>& readyQ, const std::vector<Process>& ioQ, const std::vector<Process>& complete, const Process& onCPU, const int numProcess, const bool hasTimeLimit); // print out RT, WT, and TT for each process
void printWhenNewProcessLoaded(int sysTime, const std::vector<Process>& readyQ, const std::vector<Process>& ioQ, const std::vector<Process>& complete, const Process& onCPU, const bool CPUidle); // print information of each queue when a new process is just loaded onto CPU
